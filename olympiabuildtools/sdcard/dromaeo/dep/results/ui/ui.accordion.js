...
$.extend($.ui.accordion, {
	...
	animations: {
		slide: function(settings, additions) {
			...
			settings.toHide.filter(":hidden").css({ overflow: "hidden" }).each(settings.finished).end().filter(":visible").animate({height:"hide"},{
				step: function(n){
...
$.fn.extend({
	...
	accordion: function(settings) {
		...
		var container = this,
			headers = container.find("> li >" + settings.header),
			active = findActive(settings.active),
			running = 0;

		if ( settings.fillSpace ) {
			var maxHeight = parseInt(this.parent().css("height"));
			headers.each(function() {
				maxHeight -= $(this).outerHeight({ margin: true }) +
					parseInt($(this).parent().css("marginTop")) +
					parseInt($(this).parent().css("marginBottom"));
			});
			var maxPadding = 0;
			var bodies = headers.nextUntil(settings.header).each(function() {
				maxPadding = Math.max(maxPadding, $(this).innerHeight() - $(this).height());
			});
			
			if ( settings.minHeight && maxHeight - maxPadding < settings.minHeight ) {
				bodies.height( settings.minHeight );
				container.css("overflow", "auto");	
			} else
				bodies.height( maxHeight - maxPadding );
