...
#include "GCControllerBlackBerry.h"
#include "IntSize.h"
...
static WTF::String drtRangeDescription(WebCore::Range* range)
{
    if (!range)
        return "(null)";
    return WTF::String::format("range from %ld of %s to %ld of %s", range->startOffset(), drtDumpPath(range->startContainer()).utf8().data(), range->endOffset(), drtDumpPath(range->endContainer()).utf8().data());
}

static WTF::String drtFrameDescription(WebCore::Frame* frame)
{
    WTF::String name = frame->tree()->name().string();
    if (frame == mainFrame) {
...
{
    WTF::String sdcardPath = "/media/usd";
...
{
    gLayoutTestController = LayoutTestController::create((kSDCLayoutTestsURI + *m_currentTest).utf8().data(), "");
...
static const char* fontFamilyNames[] = {
    "Ahem",
    "ColorBits",
    "WebKitWeightWatcher100",
    "WebKitWeightWatcher200",
    "WebKitWeightWatcher300",
    "WebKitWeightWatcher400",
    "WebKitWeightWatcher500",
    "WebKitWeightWatcher600",
    "WebKitWeightWatcher700",
    "WebKitWeightWatcher800",
    "WebKitWeightWatcher900",
    0
};

static void loadFonts()
{
    WTF::String fontDir("/media/usd/fonts/");

    Olympia::Platform::Text::setLoadAllFonts();

    for (int i = 0; fontFileNames[i]; i++) {
        ASSERT(fontFamilyNames[i]);
        WTF::String fontName(fontFamilyNames[i]);
        WTF::String fontPath(fontDir + fontFileNames[i]);
...
void DumpRenderTree::dump()
{
    ...
    WTF::String data =  dumpAsText ? dumpFramesAsText(mainFrame) : m_page->d->renderTreeDump();
...
WTF::String DumpRenderTree::pageGroupName() const
{
   ASSERT(m_page);
