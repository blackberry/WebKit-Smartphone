...
void XMLDocumentParser::endElementNs()
{
...
#endif
    {
        ...
        if (!scriptHref.isEmpty()) {
            ...
            scriptElement->executeScript(ScriptSourceCode(scriptElement->scriptContent(), document()->url(), m_scriptStartLine));
...
#if COMPILER(MSVC) || COMPILER(RVCT) || PLATFORM(OLYMPIA)
    char m[1024];
...
#if ENABLE(XHTMLMP)
        // FIXME: This is a temporary fix for the bug that we cannot handle other XHTML DTD's when ENABLE(XHTMLMP) is true
        // Document::isXHTMLMPDocument returns true without checking DTD. If we don't set m_hasDocTypeDeclaration, ENABLE(XHTMLMP) code
        // in XMLTokenizer::startElementNs will raise an error.
        // I'm not sure this is logically correct. Charles is working on this problem and will provide a perfect solution soon.
        if (!extId.isEmpty())
