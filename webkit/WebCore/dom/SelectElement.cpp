...
void SelectElement::listBoxDefaultEventHandler(SelectElementData& data, Element* element, Event* event)
{
    ...
    // an external widget will handle the actual selections. This prevents options from being selected
    // prior to displaying the widget. The contents of the listbox are for display only.

    // Give the element focus on mouse down so that the onFocusEvent is fired at the correct time.
