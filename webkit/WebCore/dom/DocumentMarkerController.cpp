...
void DocumentMarkerController::addMarker(Range *range, DocumentMarker::MarkerType type, AttributeTextStyle attributeTextStyle, String description)
{
    // Use a TextIterator to visit the potentially multiple nodes the range covers.
    for (TextIterator markedText(range); !markedText.atEnd(); markedText.advance()) {
        RefPtr<Range> textPiece = markedText.range();
        int exception = 0;
        DocumentMarker marker = {type, textPiece->startOffset(exception), textPiece->endOffset(exception), description, false, attributeTextStyle};
        addMarker(textPiece->startContainer(exception), marker);
    }
}
    ...
    if (!vectorPair) {
        ...
        for (i = 0; i < numMarkers; ++i) {
            ...
            if (marker.startOffset > newMarker.startOffset)
                break;
            if (marker.typesMatch(newMarker) && marker.endOffset >= newMarker.startOffset) {
        ...
        while (j < numMarkers) {
            ...
            if (marker.startOffset > newMarker.endOffset)
                break;
            if (marker.typesMatch(newMarker)) {
                markers.remove(j);
                rects.remove(j);
