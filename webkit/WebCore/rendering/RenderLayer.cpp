...
void RenderLayer::computeRepaintRects()
{
    RenderBoxModelObject* repaintContainer = renderer()->containerForRepaint();
#if !(USE(ACCELERATED_COMPOSITING))
    IntRect origRect = m_repaintRect;
#endif
    m_repaintRect = renderer()->clippedOverflowRectForRepaint(repaintContainer);
    m_outlineBox = renderer()->outlineBoundsForRepaint(repaintContainer);
#if !(USE(ACCELERATED_COMPOSITING))
    // Repaint, because FrameView::repaintFixedElementsAfterScrolling() does not do this.
    renderer()->repaintUsingContainer(repaintContainer, origRect);
    renderer()->repaintUsingContainer(repaintContainer, m_repaintRect);
#endif
}
...
void RenderLayer::updateHoverActiveState(const HitTestRequest& request, HitTestResult& result)
{
    ...
    bool isHoveredNode = true;
#if PLATFORM(OLYMPIA)
    isHoveredNode = !request.fingerUp();
#endif
    size_t addCount = nodesToAddToChain.size();
    for (size_t i = 0; i < addCount; ++i) {
        nodesToAddToChain[i]->setActive(request.active());
        nodesToAddToChain[i]->setHovered(isHoveredNode);
    }
}
