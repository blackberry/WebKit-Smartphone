...
RenderText::RenderText(Node* node, PassRefPtr<StringImpl> str)
     ...
     , m_endMinWidth(0)
#if PLATFORM(OLYMPIA)
     , m_startUnmaskSecure(0)
     , m_lengthUnmaskSecure(0)
#endif
     , m_hasTab(false)
     , m_linesDirty(false)
...
#if PLATFORM(OLYMPIA)
        // Note:  This change is part of password unmasking and will not be upstreamed.
        if (style()->textSecurity() != TSNONE && m_lengthUnmaskSecure)
            m_text = m_text.replace(m_startUnmaskSecure, m_lengthUnmaskSecure, m_originalText.substring(m_startUnmaskSecure, m_lengthUnmaskSecure));
#endif
...
#if PLATFORM(OLYMPIA)
// Note:  This change is part of password unmasking and will not be upstreamed.
// Triggers the unmask for the specified range of the field and updates the text.
void RenderText::setUnmaskSecure(int start, int length)
{
    m_startUnmaskSecure = start;
    m_lengthUnmaskSecure = length;

    setText(m_originalText.impl(), true);
}
#endif
