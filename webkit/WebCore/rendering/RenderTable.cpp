...
//     if ( oldWidth != width() || columns.size() + 1 != columnPos.size() )
    ...
    for (RenderObject* child = firstChild(); child; child = child->nextSibling()) {
        if (child->isTableSection()) {
            ...
            if (view()->shouldReflow()) // Might have children that need to reflow
    ...
    addAbsoluteLayoutOverflow(IntRect(leftBorderOverflow, topBorderOverflow, rightBorderOverflow - leftBorderOverflow, bottomBorderOverflow - topBorderOverflow));
