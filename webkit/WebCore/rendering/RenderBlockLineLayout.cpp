        ...
        for (BidiRun* r = firstRun; r; r = r->next()) {
            ...
            if (r->m_object->isText()) {
                ...
                if (r->m_object->style()->collapseWhiteSpace()) {
                    spaceAdd = (availableWidth > totWidth ? availableWidth - totWidth : 0) * spaces / numSpaces;
                                              ...
                                              bool& hyphenated, EClear* clear)
{
    ...
    if (frameView->flowTextToFit(node())) {
        int reflowWidth = frameView->reflowWidth();
        reflowWidth -= 2 * 4; // Fixed margin
        if (reflowWidth && width > reflowWidth)
            width = reflowWidth;
    }
#endif
...
void RenderBlock::addOverflowFromInlineChildren()
{
    for (RootInlineBox* curr = firstRootBox(); curr; curr = curr->nextRootBox()) {
        addAbsoluteLayoutOverflow(curr->layoutOverflowRect());
        addAbsoluteVisualOverflow(curr->visualOverflowRect());
