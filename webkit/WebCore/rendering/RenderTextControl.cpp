...
#if PLATFORM(OLYMPIA)
// Note:  This change is part of password unmasking and will not be upstreamed.
// This function walks through the individual RenderText's that are part of the
// control and figures out what text subset within the fields needs to be unmasked
// based on the overall text that should be unmasked.
void RenderTextControl::setInnerTextUnmaskSecure(int start, int length)
{
    if (!m_innerText)
        return;

    int relativeOffset = start;
    int remainingLength = length;

    // TODO:  What if there is a brTag in the field we want to unmask?
    for (Node* n = m_innerText.get(); n; n = n->traverseNextNode(m_innerText.get())) {
        if (!n->isTextNode())
            continue;

        RenderText* renderText = toRenderText(n->renderer());
        int blockTextLength = renderText->textLength();

        if (!remainingLength || !blockTextLength) {
            renderText->setUnmaskSecure(0, 0);
        } else if (blockTextLength < relativeOffset) {
            // This renderText ends before unmaskStart
            renderText->setUnmaskSecure(0, 0);
            relativeOffset -= blockTextLength;
        } else if (blockTextLength >= relativeOffset + remainingLength) {
            // Contained entirely within this renderText.
            renderText->setUnmaskSecure(relativeOffset, remainingLength);
            remainingLength = 0;
        } else {
            // Contains a portion of the text.
            int blockUnmaskLength = blockTextLength - relativeOffset;

            renderText->setUnmaskSecure(relativeOffset, blockUnmaskLength);

            relativeOffset = 0;
            remainingLength -= blockUnmaskLength;
        }
    }
}
#endif
