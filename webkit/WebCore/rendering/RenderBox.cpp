...
void RenderBox::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (isBody()) {
        ...
        view()->setBodyIsLTR(style()->direction() == LTR);
    }
}
...
int RenderBox::calcReplacedHeightUsing(Length height) const
{
    switch (height.type()) {
        ...
        case Percent:
        {
            ...
            if (!document()->inQuirksMode() && cb->style()->height().isAuto() && !isPositioned() && !cb->isRenderView() && !cb->isTableCell()) {
...
{   
    ...
    if (frameView && !frameView->fixedReportedSize().isEmpty() && style()->position() == FixedPosition)
...
void RenderBox::addShadowOverflow()
{
    ...
    addAbsoluteVisualOverflow(IntRect(overflowLeft, overflowTop, overflowRight - overflowLeft, overflowBottom - overflowTop));
...
{
    // Propagate absolute layout overflow rect
    IntRect absoluteChildLayoutOverflowRect = child->absoluteLayoutOverflowRect();
    absoluteChildLayoutOverflowRect.move(delta);
    addAbsoluteLayoutOverflow(absoluteChildLayoutOverflowRect);

    // Propagate absolute visual overflow rect
    IntRect absoluteChildVisualOverflowRect = child->absoluteVisualOverflowRect();
    absoluteChildVisualOverflowRect.move(delta);
    addAbsoluteVisualOverflow(absoluteChildVisualOverflowRect);
...
void RenderBox::addAbsoluteLayoutOverflow(const IntRect& rect)
{
    IntRect borderBox = borderBoxRect();
    if (borderBox.contains(rect))
        return;

    if (!m_absoluteOverflow)
        m_absoluteOverflow.set(new RenderOverflow(borderBox));

    m_absoluteOverflow->addLayoutOverflow(rect);
}

void RenderBox::addAbsoluteVisualOverflow(const IntRect& rect)
{
    IntRect borderBox = borderBoxRect();
    if (borderBox.contains(rect))
        return;
    ...
    m_absoluteOverflow->addVisualOverflow(rect);
}
