...
#if ENABLE(OFFLINE_WEB_APPLICATIONS) && OS(OLYMPIA)
    // Don't use cache for resources listed as Fallback and/or Network
    Frame* f = frame();
    if (f && f->loader()->documentLoader()->applicationCacheHost()->isFallbackOrNetworkResource(fullURL))
        policy = CachePolicyReload;
#endif
...
CachedResource* CachedResourceLoader::requestResource(CachedResource::Type type, const String& url, const String& charset, bool isPreload)
{
    ...
    if (cache()->disabled() || !settings || !settings->useCache()) {
#else
    if (cache()->disabled()) {
#endif
