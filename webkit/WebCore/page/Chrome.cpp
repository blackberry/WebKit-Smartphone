...
void Chrome::invalidateContentsForSlowScroll(const IntSize& scrollDelta, const IntRect& updateRect, bool immediate, const ScrollView* scrollView)
{
    m_client->invalidateContentsForSlowScroll(scrollDelta, updateRect, immediate, scrollView);
}
...
{
    m_client->layoutFinished(frame);
}

void Chrome::overflowExceedsContentsSize(Frame* frame) const
{
    m_client->overflowExceedsContentsSize(frame);
}
#endif

#if ENABLE(VIEWPORT_REFLOW)
int Chrome::reflowWidth() const
{
    return m_client->reflowWidth();
}
#endif
