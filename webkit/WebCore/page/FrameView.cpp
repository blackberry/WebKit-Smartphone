...
void FrameView::reset()
{
    ...
    m_contentsSizeFromHistory = IntSize();
#if PLATFORM(OLYMPIA)
    m_firstLayoutScheduleStartTime = 0;
#endif
}
...
void FrameView::setContentsSize(const IntSize& contentsSize)
{
    // Expand the reported size to a minimum of the contents size from history
    // if loading is not complete...
    IntSize size = contentsSize;
    if (!m_frame->loader()->isComplete())
        size = size.expandedTo(m_contentsSizeFromHistory);

    if (size == this->contentsSize())
        return;
...
void FrameView::setContentsSizeFromHistory(const IntSize& contentsSize)
{
    m_contentsSizeFromHistory = contentsSize;
    adjustViewSize();
}

void FrameView::checkRelayoutForContentsSizeFromHistory()
{
    ...
    // Called by the frame loader once loading is complete to determine if the final
    // contents size is different from the contents size from history and if so
    // schedule a relayout
    if (!m_contentsSizeFromHistory.isEmpty() && m_contentsSizeFromHistory != IntSize(root->rightLayoutOverflow(), root->bottomLayoutOverflow()))
        scheduleRelayout();
}
...
void FrameView::adjustViewSize()
{
    ...
    bool overflowExceedsContentsSize = root->rightAbsoluteVisibleOverflow() > root->rightLayoutOverflow() ||
                                       root->bottomAbsoluteVisibleOverflow() > root->bottomLayoutOverflow();
    if (overflowExceedsContentsSize) {
        Page* page = frame() ? frame()->page() : 0;
        if (page)
            page->chrome()->overflowExceedsContentsSize(frame());
    }
#endif

    ScrollView::setScrollOriginX(-root->leftLayoutOverflow());
    setContentsSize(IntSize(root->rightLayoutOverflow() - root->leftLayoutOverflow(), root->bottomLayoutOverflow()));
}
...
{
// FIXME: The Olympia port just ignores static backgrounds for now, but the
// proper thing to do would be to introduce ENABLE(IGNORE_FIXED_STYLES) and
// instrument render tree to ignore/not paint anything fixed when this is set.
#if !PLATFORM(OLYMPIA)
...
#if PLATFORM(OLYMPIA)
int FrameView::firstLayoutScheduleElapsedTime() const
{
    return static_cast<int>((currentTime() - m_firstLayoutScheduleStartTime) * 1000);
}
#endif
...
#if PLATFORM(OLYMPIA)
    // We schedule the first layout much faster than the rest
    // so that we can display something interesting to the user.
    // After this, go back to normal scheduling...
    if (m_firstVisuallyNonEmptyLayoutCallbackPending) {
        if (Page* page = m_frame->page()) {
            if (page->settings()->shouldUseFirstScheduledLayoutDelay()) {
                m_firstLayoutScheduleStartTime = currentTime();
                delay = page->settings()->firstScheduledLayoutDelay();
            }
        }
    }
#endif
...
#if PLATFORM(OLYMPIA)
        // We distinguish between the layout size and the actual visible size
        // and send resize events when the latter changes.
#else
        if (resized)
...
int FrameView::reflowWidth() const
{
    int reflowWidth = 0;

    Page* page = frame() ? frame()->page() : 0;
    if (page)
        reflowWidth = page->chrome()->reflowWidth();

    return reflowWidth > 0 ? reflowWidth : visibleWidth();
}

bool FrameView::flowTextToFit(Node* node) const
{
    Settings *settings = frame()->settings();

    if (settings) {
        // Return right away if the setting is disabled to avoid further computation.
        if (!settings->isTextReflowEnabled())
            return false;

        bool doReflow = true;
        if (node && node->renderer()) {
            // Don't reflow a fixed positioned element
            if (node->renderer()->isPositioned())
                return false;

            // don't reflow justified text
            if (node->renderer()->style()->textAlign() == JUSTIFY)
                return false;

            // Check for fixed height containers. If we reflow in this case, it could cause text overlap or
            // even disappearing text where overflow=hidden.
            RenderObject* obj = node->renderer();
            while (obj) {
                // We special case tables here because fixed height tables can actually adjust their height to be
                // greater than the specified height in order to fit the content.
                // The table cell's declared line height less than the font's height can potentially cause text overlap if reflowed.
                if (obj->style()->height().isFixed() &&
                        !obj->isTable() && !obj->isTableCell() && !obj->isTableCol() &&
                        !obj->isTableRow() && !obj->isTableSection() ||
                        obj->isTableCell() && obj->style()->computedLineHeight() < obj->style()->font().height()) {
                    doReflow = false;
                    break;
                }
                if (obj->isPositioned())
