...
void jpegInitializeDestination(j_compress_ptr compressData)
{
...
boolean jpegEmptyOutputBuffer(j_compress_ptr compressData)
{
...
void jpegTerminateDestination(j_compress_ptr compressData)
{
    JPEGDestinationManager* dest = static_cast<JPEGDestinationManager*>(compressData->dest);
    dest->m_dump.append(dest->m_buffer.data(), dest->m_buffer.size() - dest->free_in_buffer);
}

void jpegErrorExit(j_common_ptr compressData)
{
