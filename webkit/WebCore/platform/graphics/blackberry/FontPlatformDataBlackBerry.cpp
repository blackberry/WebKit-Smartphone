    ...
    , m_descentOverride(0.0)
{
    ...
    String locale = setlocale(LC_ALL, 0);
    if (locale.contains("ja"))
        spec.m_hanStyle = Olympia::Platform::Text::JapaneseHanStyle;
    else if (locale.contains("ko"))
        spec.m_hanStyle = Olympia::Platform::Text::KoreanHanStyle;
    else if (locale.contains("zh_TW") || locale.contains("zh_HK"))
        spec.m_hanStyle = Olympia::Platform::Text::TraditionalHanStyle;
    else if (locale.contains("zh_CN"))
        spec.m_hanStyle = Olympia::Platform::Text::SimplifiedHanStyle;
    ...
    if (!family.isEmpty() && family != "-webkit-last-resort") {
        ...
        if (!familyMatches) {
            ...
            if (equalIgnoringCase(family, "Calibri")
                    ...
                    || equalIgnoringCase(family, "MS UI Gothic")
               )
                familyMatches = true;
        }
...
unsigned FontPlatformData::hash() const
{
    ...
    hashString[Olympia::Platform::Text::MAX_FONT_NAME - 5] ^=
