...
CSSSelector* CSSParser::createFloatingSelector()
{
    ...
    m_floatingSelectors.add(selector, 1);
    return selector;
}
...
CSSSelector* CSSParser::connectFloatingSelector(CSSSelector* ownerSelector, CSSSelector* sinkingSelector, CSSSelector::Relation relation)
{
    // To avoid stack overflow, set the limit for the number of CSSSelector chaining.
    static const int maxChainingLength = 256;
    ASSERT(ownerSelector);
    ASSERT(m_floatingSelectors.contains(ownerSelector));
    if (!sinkingSelector)
        return ownerSelector;
    ASSERT(m_floatingSelectors.contains(sinkingSelector));
    int newChainingLength = m_floatingSelectors.get(ownerSelector) + m_floatingSelectors.take(sinkingSelector);
    if (newChainingLength > maxChainingLength) {
        // This should not happen unless there is an intention to crash WebKit.
        // It is okay to get rid of selectors in this case.
        delete sinkingSelector;
        return ownerSelector;
    }
    m_floatingSelectors.set(ownerSelector, newChainingLength);
    CSSSelector* end = ownerSelector;
    while (end->tagHistory())
        end = end->tagHistory();
    end->setTagHistory(sinkingSelector);
    end->m_relation = relation;
    return ownerSelector;
}
