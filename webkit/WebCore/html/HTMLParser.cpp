...
#define DEBUG_STRINGPATTERN_MATCH 0
#if DEBUG_STRINGPATTERN_MATCH
#include "Logging.h"
#endif
#endif
...
// Run StringPatterns over inputText, adding link to DOM tree for first matched text.
// Returns leftover text after matched string.
PassRefPtr<Text> HTMLParser::parseNextStringPatternMatch(Text* inputText)
{
    ASSERT(inputText);

    if (inputText->length() < 8) {
        // don't match very short strings, since none of the patterns will match
        return 0;
    }

    String s = inputText->wholeText();

#if DEBUG_STRINGPATTERN_MATCH
    LOG(Loading, "  matching patterns for \"%s\"\n", s.latin1().data());
#endif

    const Vector<String>& formats = m_document->detectedFormats();
    StringPattern::Match bestMatch;
    StringPattern* matchedPattern = 0;
    for (int i = 0; i < formats.size(); ++i) {
        StringPattern& pattern = StringPattern::getPattern(formats.at(i));

        StringPattern::Match match;
        pattern.findMatch(s, 0, match);
        if (match.beginIndex != -1 && match.endIndex != -1) {
            // Match found. Earlier, longer matches take precedence.
            if (match.beginIndex < bestMatch.beginIndex || (match.beginIndex == bestMatch.beginIndex && match.endIndex > bestMatch.endIndex) || bestMatch.beginIndex < 0) {
                bestMatch = match;
                matchedPattern = &pattern;

                if (match.beginIndex == 0 && match.endIndex == s.length() - 1)  // the whole string is matched, so nothing can be earlier or longer
                    break;

#if DEBUG_STRINGPATTERN_MATCH
                String matchedString = s.substring(match.beginIndex, match.endIndex - match.beginIndex + 1);
                LOG(Loading, "  matched %s, \"%s\"\n", matchedPattern->type().string().latin1().data(), matchedString.latin1().data());
#endif
            }
        }
    }
    ...
    RefPtr<Text> outputText = inputText;
    ExceptionCode ec = 0;
    RefPtr<HTMLAnchorElement> myAnchor = static_cast<HTMLAnchorElement*>(m_document->createElementNS(xhtmlNamespaceURI, aTag.toString(), ec).get());
    if (m_current->childAllowed(myAnchor.get())) {
        RefPtr<Text> patternText = inputText->splitText(bestMatch.beginIndex, ec);
        RefPtr<Text> afterPatternText = patternText->splitText(bestMatch.endIndex - bestMatch.beginIndex + 1, ec);

        myAnchor->addChild(patternText.get());
        String patternLink = matchedPattern->linkPrefix();
        patternLink.append(patternText->wholeText());
        myAnchor->setHref(patternLink);

        m_current->appendChild(inputText, ec);
        m_current->appendChild(myAnchor.get(), ec);
        outputText = afterPatternText;
    } else
        return 0;

    return outputText.release();
}

PassRefPtr<Node> HTMLParser::parseStringPatterns(Node* inputNode)
{
    ASSERT(inputNode);
    ASSERT(inputNode->isTextNode());

    RefPtr<Node> outputNode = inputNode;

    bool shouldParseStringPatterns = true;
    RefPtr<Node> parentNode = m_current;
    do {
        if (parentNode->isLink() || parentNode->isCommentNode() || parentNode->hasTagName(scriptTag) || (parentNode->isHTMLElement() && (static_cast<HTMLElement*>(parentNode.get()))->isFormControlElement()) || parentNode->hasTagName(styleTag) || parentNode->hasTagName(ttTag) || parentNode->hasTagName(preTag) || parentNode->hasTagName(codeTag) || parentNode->hasTagName(titleTag)) {
            shouldParseStringPatterns = false;
            break;
        }
        parentNode = parentNode->parentNode();
    } while (parentNode);

    if (shouldParseStringPatterns) {
        if (m_current->isHTMLElement()) {
            RefPtr<Node> linkifiedNodeRemnants;
            do {
                linkifiedNodeRemnants = parseNextStringPatternMatch(static_cast<Text*>(outputNode.get()));
                if (linkifiedNodeRemnants)
                    outputNode = linkifiedNodeRemnants;
            } while (linkifiedNodeRemnants);
        }
    }
    return outputNode.release();
}
...
#if PLATFORM(OLYMPIA)
    if (n->isTextNode() && parentAttached && m_document->isFormatDetectionEnabled()) {
#if DEBUG_STRINGPATTERN_MATCH
        LOG(Loading, "Parsing StringPatterns\n");
#endif
        protectNode = parseStringPatterns(n).get();
        n = protectNode.get();
#if DEBUG_STRINGPATTERN_MATCH
        LOG(Loading, "Done parsing StringPatterns\n");
#endif
    }
#endif
