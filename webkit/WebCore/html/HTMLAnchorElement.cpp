...
#if !PLATFORM(GTK) && !PLATFORM(QT) && !PLATFORM(EFL) && !PLATFORM(OLYMPIA)
    if (isLink())
...
#if OS(OLYMPIA)
            handleLinkClick(event, document(), url, getAttribute(targetAttr), hasRel(RelationNoReferrer), text());
#else
...
#if OS(OLYMPIA)
void handleLinkClick(Event* event, Document* document, const String& url, const String& target, bool hideReferrer, const String& text)
{
    event->setDefaultHandled();
    ...
    ResourceRequest request(document->completeURL(url));
    request.setAnchorText(text);
    frame->loader()->urlSelectedWithRequest(request, target, event, false, false, true, hideReferrer ? NoReferrer : SendReferrer);
}
#else
