...
class BlackBerryAnimationBase {
    public:
        enum ShapingFunction { Linear, EaseInCurve, EaseOutCurve };

        BlackBerryAnimationBase();
        ~BlackBerryAnimationBase();
        ...
        void animationTimerFired(WebCore::Timer<BlackBerryAnimationBase>*);

        bool initializeBuffer(const WebCore::IntSize& size);
        void destroyBuffer();
        Olympia::Platform::Graphics::Buffer* buffer() { return m_buffer; }

    private:
        WebCore::Timer<BlackBerryAnimationBase> m_timer;
        ...
        Olympia::Platform::Graphics::Buffer* m_buffer;
};

template <typename OlympiaAnimationControlClass> class OlympiaAnimation : public BlackBerryAnimationBase {
    public:
