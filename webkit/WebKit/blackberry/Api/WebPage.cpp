...
void WebPagePrivate::init(const WebString& pageGroupName)
{
...
#if ENABLE(INSPECTOR)
    inspectorClient = new InspectorClientBlackBerry();
#endif
    ...
    //                  dragClient, inspectorClient, /*WebCore::PluginHalterClient*/0, 0, 0, 0);

    m_page->setGroupName(pageGroupName);
    m_page->setCustomHTMLTokenizerChunkSize(256);
    m_page->setCustomHTMLTokenizerTimeDelay(0.300);

    m_webSettings = new WebSettings(m_page, pageGroupName);
...
bool WebPagePrivate::zoomAboutPoint(double unclampedScale, const WebCore::FloatPoint& anchor, bool enforceScaleClamping, bool forceRendering, bool isRestoringZoomLevel)
{
    ...
    *m_transformationMatrix = zoom;
    ...
    m_client->zoomChanged(m_webPage->isMinZoomed(), m_webPage->isMaxZoomed(), !shouldZoomOnEscape(), currentScale());
    m_shouldZoomToFit = isZoomedToFitScreen();
    return true;
}
...
bool WebPagePrivate::scheduleZoomAboutPoint(double unclampedScale, const FloatPoint& anchor, bool enforceScaleClamping, bool forceRendering)
{
    ...
    Olympia::Platform::Graphics::Buffer* tempBuffer = Olympia::Platform::Graphics::createBuffer(
        requiredSize, Olympia::Platform::Graphics::TemporaryBuffer);
    ...
    // See RIM Bug #641
    renderBitmapZoomFrame(scale, mapToTransformedFloatPoint(anchor), tempBuffer);
    Olympia::Platform::Graphics::destroyBuffer(tempBuffer);
    ...
    // If the resize timer is active, we need to reschedule it to fire after the delayed zoom
    // or it will fire at a time when we have initiated but not completed a zoom and thus
    // JavaScript will get the wrong innerWidth/innerHeight when asking for it in the onresize handler.
    if (m_deferredResizeEventTimer->isActive())
        m_deferredResizeEventTimer->startOneShot(deferredResizeEventInterval + delayedZoomInterval);
...
{
    if (!m_webPage->isVisible() && !m_webSettings->isDirectRenderingToCanvasEnabled())
        return;
    ...
    const IntPoint dstPoint(contentsRect.x() - surfaceOffset.x(), contentsRect.y() - surfaceOffset.y());
    const IntRect srcRect(dstPoint, contentsRect.size());

    if (m_webSettings->isDirectRenderingToCanvasEnabled()) {
        m_client->lockCanvas();
        m_client->blitToCanvasFromDrawingSurface(dstPoint, srcRect);
        m_client->unlockCanvas();
    } else
        Olympia::Platform::Graphics::blitToBufferFromDrawingSurface(tile->buffer(), dstPoint, srcRect);
}

void WebPagePrivate::blitToCanvas(const IntRect& dst, const Olympia::Platform::Graphics::Buffer* srcBuffer, const IntRect& src)
{
    IntRect dstRect = dst;
    IntRect srcRect = src;
    ...
    m_client->blitToCanvas(dstRect.location(), srcBuffer, srcRect);
}

void WebPagePrivate::stretchBlitToCanvas(const IntRect& dstRect,
                                         const Olympia::Platform::Graphics::Buffer* srcBuffer, const IntRect& srcRect)
{
    m_client->stretchBlitToCanvas(dstRect, srcBuffer, srcRect);
}

void WebPagePrivate::blendOntoCanvas(const IntRect& dst,
                                     const Olympia::Platform::Graphics::Buffer* srcBuffer,
                                     const Olympia::Platform::Graphics::Buffer* srcAlphaBuffer,
                                     const IntRect& src, char globalAlpha)
{
    IntRect dstRect = dst;
    IntRect srcRect = src;
    ...
    m_client->blendOntoCanvas(dstRect.location(), srcBuffer, srcAlphaBuffer, srcRect, globalAlpha);
}
...
void WebPagePrivate::clearCanvas(const WebCore::IntRect& rect, unsigned char red, unsigned char green, unsigned char blue)
{
    m_client->clearCanvas(rect, red, green, blue);
}
...
{
    m_mainFrame->view()->scrollPositionChangedViaPlatformWidget();
}
...
void WebPagePrivate::zoomToInitialScaleOnLoad()
{
    ...
    if (shouldZoom && loadState() == Committed) {
        ...
        m_shouldZoomToFit = m_webSettings->isZoomToFitOnLoad();
    }
...
Context WebPagePrivate::getContextForNode(Node* node)
{
    ...
    if (node->isHTMLElement() && node->hasTagName(HTMLNames::imgTag)) {
        ...
        WTF::String url = imageElement->getAttribute(HTMLNames::srcAttr).string();
    ...
    if (obj && obj->isText()) {
        types.push_back(Context::ContextTypeText);
        info.push_back(WTF::String(toRenderText(obj)->text()));
    }
...
WebCore::IntSize WebPagePrivate::fixedLayoutSize(bool snapToIncrement) const
{
    ...
    if (m_webSettings->isZoomToFitOnLoad()) {
        ...
        if (m_mainFrame->view() && !contentsSize().isEmpty())
            ...
            minWidth = (m_mainFrame->orientation() % 180 == 0) ? settings->screenWidth() : settings->screenHeight();
#else
            minWidth = settings->screenWidth();
#endif
        }
    }
...
WebCore::IntRect WebPagePrivate::blockZoomRectForNode(Node* node)
{
    ...
    m_client->clearCanvas(renderRect, 0, 0, 0);
    m_client->clearCanvas(originalRenderRect, 120, 120, 120);
...
void WebPagePrivate::animateBlockZoom()
{
    ...
    m_blockZoomAnimation->setShapingFunction(BlackBerryAnimationBase::EaseOutCurve);
...
void WebPagePrivate::animateZoomBounceBack(const FloatPoint& lastBouncePoint)
{
    ...
    m_zoomBounceBackAnimation->setShapingFunction(BlackBerryAnimationBase::Linear);
...
void WebPagePrivate::animationZoomBounceBackFrameChanged(Olympia::WebKit::OlympiaAnimation<WebPagePrivate>*, double currentValue, double previousValue)
{
    ...
    if (currentValue == -1.0) {
        ...
        Olympia::Platform::Graphics::destroyBuffer(m_bitmapZoomBuffer);
        m_bitmapZoomBuffer = 0;
        return;
    }
...
void WebPagePrivate::renderBitmapZoomFrame(double scale, const FloatPoint& anchor /* backingstore coordinates */, Olympia::Platform::Graphics::Buffer* tempBuffer /* = 0 */)
{
    // Need to invert the previous transform to anchor the viewport.
    ...
    if (!tempBuffer) {
        tempBuffer = m_bitmapZoomBuffer;
    }
    m_backingStore->d->blitScaledArbitraryRectFromBackingStoreToScreen(srcRect, IntRect(IntPoint(0, 0), transformedVisibleContentsRect().size()), tempBuffer);
}
...
bool WebPage::singleTouchEvent(Olympia::Platform::SingleTouchEvent& event)
{
    d->m_lastUserEventTimestamp = currentTime();

    event.m_pos = d->mapFromTransformed(event.m_pos);
    event.m_screenPos = d->mapFromTransformed(event.m_screenPos);
...
void WebPagePrivate::stopZoomBounceBackAnimationIfNeeded()
{
    if (m_zoomBounceBackAnimation && m_zoomBounceBackAnimation->isRunning())
        m_zoomBounceBackAnimation->stop();
}
...
bool WebPage::keyEvent(Olympia::Platform::KeyboardEvent::Type type, const unsigned short character, bool shiftDown, bool altDown)
{
    if (!d->m_mainFrame->view())
        return false;

    ASSERT(d->m_page->focusController());
    return d->m_inputHandler->handleKeyboardInput(toWebCorePlatformKeyboardEventType(type), character, shiftDown, altDown);
}
...
void WebPage::selectionCancelled()
{
    ...
    // Trigger a repaint of the visible selection rect as it frequently isn't updated
    // See RIM Bug #1242.
    d->m_backingStore->d->repaint(IntRect(IntPoint(0, 0), d->actualVisibleSize()), true /*contentChanged*/, false /*immediate*/, false /*repaintContentOnly*/);
}
...
Olympia::Platform::IntRect WebPagePrivate::selectionStartCaretRect()
{
    WebCore::IntRect rect = mapToTransformed(m_selectionHandler->selectionStartCaretRect());
    clipToTransformedContentsRect(rect);
    return Olympia::Platform::IntRect(rect);
}

// returned caret rect in transformed coordinates
Olympia::Platform::IntRect WebPagePrivate::selectionEndCaretRect()
{
    WebCore::IntRect rect = mapToTransformed(m_selectionHandler->selectionEndCaretRect());
    clipToTransformedContentsRect(rect);
    return Olympia::Platform::IntRect(rect);
}
...
{
    d->stopZoomBounceBackAnimationIfNeeded();
    if (d->contentsSize().isEmpty() || !d->isUserScalable())
        return false;
    ...
    if (!d->m_bitmapZoomBuffer) {
        d->m_bitmapZoomBuffer = Olympia::Platform::Graphics::createBuffer(d->transformedVisibleContentsRect().size(), Olympia::Platform::Graphics::TemporaryBuffer);
    }
    ...
    if (d->m_bitmapScale < minimumScale)
        ...
        d->m_userPerformedManualZoom = true;
        d->m_userPerformedManualScroll = true;
        Olympia::Platform::Graphics::destroyBuffer(d->m_bitmapZoomBuffer);
...
{
    return d->zoomedWidth(d->currentScale()) == d->zoomedWidth(d->maximumScale()) || !d->isUserScalable();
}

bool WebPage::isMinZoomed() const
{
    return d->zoomedWidth(d->currentScale()) == d->zoomedWidth(d->minimumScale()) || !d->isUserScalable();
}

bool WebPage::isAtInitialZoom() const
{
    return d->zoomedWidth(d->currentScale()) == d->zoomedWidth(d->initialScale()) || !d->isUserScalable();
}

bool WebPagePrivate::isZoomedToFitScreen() const
{
    return zoomedWidth(currentScale()) == zoomedWidth(zoomToFitScale()) || !isUserScalable();
}
...
{
    return d->m_selectionHandler->findNextString(WTF::String::fromUTF8(text));
}

bool WebPage::findNextUnicodeString(const unsigned short* text)
{
    return d->m_selectionHandler->findNextString(WTF::String(text));
}
