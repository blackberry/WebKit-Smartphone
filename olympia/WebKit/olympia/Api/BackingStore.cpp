...
{
    return m_webPage->isVisible() && !m_suspendRenderJobs && !m_suspendScreenUpdates && !m_renderQueue->isEmpty(!m_suspendRegularRenderJobs);
}
    ...
    for (TileMap::const_iterator it = m_tileMap.begin(); it != tileMapEnd; ++it) {
        ...
        if (backingStoreRect.contains(origin)) {
            ...
            if (m_renderQueue->regularRenderJobsPreviouslyAttemptedButNotRendered(rect)) {
                ...
                if (!tile->isCompletelyRendered() && !isCurrentVisibleJob(newIndex, tile.get(), backingStoreRect))
