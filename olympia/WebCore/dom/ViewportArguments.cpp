...
void setViewportFeature(const String& keyString, const String& valueString, Document* document, void* data)
{
    ...
    if (equalIgnoringCase(valueString, "yes"))
        ...
        if (document->page())
            value = document->page()->chrome()->client()->platformScreenSize().width();
    } else if (equalIgnoringCase(valueString, "device-height")) {
        didUseConstants = true;
        if (document->page())
            value = document->page()->chrome()->client()->platformScreenSize().height();
    ...
    if (keyString == "initial-scale")
        ...
        if (document->page() && value == document->page()->chrome()->client()->platformScreenSize().width() && !didUseConstants)
            reportViewportWarning(document, DeviceWidthShouldBeUsedWarning, keyString);
        else if (document->page() && value == document->page()->chrome()->client()->platformScreenSize().height() && !didUseConstants)
