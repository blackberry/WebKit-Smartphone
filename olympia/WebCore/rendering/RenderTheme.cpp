...
void RenderTheme::paintCaret(GraphicsContext* context, const IntRect& caretRect, const Element* rootEditableElement)
{
    ASSERT(context);
    ...
    context->fillRect(caretRect, caretColor, colorSpace);
}

void RenderTheme::repaintCaret(RenderView* view, const IntRect& caretRect, CaretVisibility)
{
    ASSERT(view);
    view->repaintRectangleInViewAndCompositedLayers(SelectionController::repaintRectForCaret(caretRect), false);
}
