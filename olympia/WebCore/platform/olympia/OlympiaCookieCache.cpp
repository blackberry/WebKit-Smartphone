...
OlympiaCookieCache& OlympiaCookieCache::instance()
{
    static OlympiaCookieCache cache(cookieCacheCapacity);
    return cache;
}

OlympiaCookieCache::OlympiaCookieCache(int capacity)
    : m_capacity(capacity)
    , m_size(0)
{
}

String OlympiaCookieCache::cookies(const String& pageGroup, const KURL& url)
{
...
bool OlympiaCookieCache::setCookies(const String& pageGroup, const KURL& url, const String& cookies)
{
    String key = url.string().lower();
    String host = url.host().lower();
    if (pageGroup.isEmpty() || key.isEmpty() || host.isEmpty())
...
int OlympiaCookieCache::purgeStorage()
{
...
void OlympiaCookieCache::clearAllCookiesForHost(const String& pageGroup, const KURL& url)
{
...
void OlympiaCookieCache::clearAllCookies()
{
    PageGroupMap::iterator groupIterator = m_groupMap.begin();
    PageGroupMap::iterator groupIteratorEnd = m_groupMap.end();
    for (; groupIterator != groupIteratorEnd; ++groupIterator) {
