...
#if PLATFORM(OLYMPIA)
    bool yieldForFirstLayout = false;
    if (m_doc && m_doc->view() && m_doc->settings()) {
        yieldForFirstLayout = m_doc->settings()->shouldUseFirstScheduledLayoutDelay()
            && !m_doc->view()->didFirstLayout()
            && m_doc->view()->layoutPending()
            && m_doc->view()->firstLayoutScheduleElapsedTime() > m_doc->settings()->firstScheduledLayoutDelay();
    }
    allowedYield = yieldForFirstLayout ? true : allowedYield;
#endif

    if (!state.loadingExtScript() && !state.forceSynchronous() && !m_executingScript && (processedCount > m_tokenizerChunkSize || allowedYield)) {
        processedCount = 0;
        bool shouldDefer = currentTime() - startTime > m_tokenizerTimeDelay;
#if PLATFORM(OLYMPIA)
        shouldDefer = shouldDefer || yieldForFirstLayout;
#endif
        if (shouldDefer) {
...
void HTMLTokenizer::timerFired(Timer<HTMLTokenizer>*)
{
    ...
    if (Page* page = m_doc->page()) {
        if (page->defersLoading()) {
            // If the page is deferred, we postpone the timer by 10 ms.
            m_timer.startOneShot(0.01);
            return;
        }
    }
...
void HTMLTokenizer::executeExternalScriptsTimerFired(Timer<HTMLTokenizer>*)
{
    ...
    if (Page* page = m_doc->page()) {
        if (page->defersLoading()) {
            // If the page is deferred, we postpone the timer by 10 ms.
            m_externalScriptsTimer.startOneShot(0.01);
            return;
        }
    }
